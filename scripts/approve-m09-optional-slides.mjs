...
const sourceRoot = path.join(ROOT, "assets", "slide-sources", "optional", "M09");
const targetIds = new Set(walk(sourceRoot)
  .filter((file) => file.endsWith(".svg"))
  .map((file) => path.basename(file, ".svg")));

if (targetIds.size !== 9) throw new Error(`Expected 9 M09 optional slides, found ${targetIds.size}`);

const manifest = JSON.parse(fs.readFileSync(MANIFEST_PATH, "utf8"));
const targets = manifest.slides.filter((slide) => targetIds.has(slide.id));
if (targets.length !== 9 || targets.some((slide) => slide.module !== "M09")) {
  throw new Error(`M09 approval target mismatch: ${targets.length}`);
}
...
console.log(`m09_optional_approval=ok count=${targets.length}`);
