...
const sourceRoot = path.join(ROOT, "assets", "slide-sources", "optional", "M08");
const targetIds = new Set(walk(sourceRoot)
  .filter((file) => file.endsWith(".svg"))
  .map((file) => path.basename(file, ".svg")));

if (targetIds.size !== 27) throw new Error(`Expected 27 M08 optional slides, found ${targetIds.size}`);

const manifest = JSON.parse(fs.readFileSync(MANIFEST_PATH, "utf8"));
const targets = manifest.slides.filter((slide) => targetIds.has(slide.id));
if (targets.length !== 27 || targets.some((slide) => slide.module !== "M08")) {
  throw new Error(`M08 approval target mismatch: ${targets.length}`);
}
...
console.log(`m08_optional_approval=ok count=${targets.length}`);
